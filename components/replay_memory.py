...
class ReplayMemory:
    ...
    def push(self, exp):
        ...
        idxs = np.array([i%self.capacity for i in range(self.nb_sampels-nb_added, self.nb_sampels)])
        if self.nb_sampels > self.capacity:
            ...
            for key in self.memory.keys():
                self.memory[key] = np.repeat(np.array([exp[key][0]]), self.capacity, axis=0)
                self.memory[key][idxs] = exp[key]
        else:
            for key in self.memory.keys():
#                self.memory[key] = np.concatenate((self.memory[key], exp[key]), axis=0 )
    ...
    def sample(self, batch_size):
        ...
        batch_size = self.anneal_bs(batch_size, 4) * batch_size
